...
function Test-QuestDBConnection {
    ...
    try {
        $url = "http://${Server}:${Port}/exec?query=SELECT%201"
...
function Invoke-QuestDBQuery {
    param(
        [string]$Query,
        [string]$Server = $QuestDBHost,
        [int]$Port = $QuestDBPort
    )

    try {
        $encodedQuery = [System.Web.HttpUtility]::UrlEncode($Query)
        $url = "http://${Server}:${Port}/exec?query=${encodedQuery}"
...
function Invoke-QuestDBScript {
    ...
    foreach ($statement in $statements) {
        ...
        try {
            Invoke-QuestDBQuery -Query $trimmed -Server $Server -Port $Port | Out-Null
            $successCount++
        }
        catch {
            $failCount++
...
function Install-QuestDB {
    ...
    if (-not (Test-QuestDBConnection -Server $QuestDBHost -Port $QuestDBPort)) {
    ...
    foreach ($migration in $pendingMigrations) {
        Write-Host "   $($migration.Version) - $($migration.Name)" -ForegroundColor Cyan
    }
    Write-Host ""
    ...
    try {
        ...
        if ($tables.dataset) {
            ...
            foreach ($table in $tables.dataset) {
                Write-Host "     $($table[0])" -ForegroundColor DarkGray
            }
        }
    }
    catch {
