# Story BUG-007.1c: Refactor ConditionProgress to Use Shared WebSocket
...
I want **ConditionProgress to use the shared wsService singleton**,
so that **there is only one WebSocket connection with proper auth, heartbeat, and reconnection**.

## Acceptance Criteria

1. **AC1:** ConditionProgress uses `wsService` singleton instead of `new WebSocket()`
2. **AC2:** Component subscribes to appropriate condition progress stream
3. **AC3:** Remove standalone WebSocket management code
4. **AC4:** Connection remains stable with proper heartbeat

## Tasks / Subtasks

- [ ] Task 1: Analyze current WebSocket usage (line 237)
  - [ ] Read ConditionProgress.integration.tsx to understand current implementation
...
- Component at `frontend/src/components/dashboard/ConditionProgress.integration.tsx:237` creates own WebSocket
...
- [Source: _bmad-output/bug-007-epic-stories.md#Story-1c]
